...
def main():
    ...
    # Create customers
    myles = Customer("Myles")
    michelle = Customer("Michelle")
    print(f"Created customers: {myles.name}, {michelle.name}")
    ...
    # Create orders
    order1 = myles.create_order(latte, 5.0)
    order2 = myles.create_order(espresso, 4.0)
    order3 = michelle.create_order(latte, 6.0)
    ...
    print(f"\nMyles' orders: {len(myles.orders())}")
    print(f"Myles' coffees: {[coffee.name for coffee in myles.coffees()]}")
